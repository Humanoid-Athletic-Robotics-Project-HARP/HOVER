...
# T-pose geometric errors (arm_scale=0.395):
#   Left_Arm_1  → L_Shoulder:  20mm  (shoulder body, mostly static wrt arm DOFs)
#   Left_Arm_3  → L_Elbow:     35mm  (elbow body — just past Left_Elbow_Pitch, so its
#                                      position directly encodes elbow bending; forces
#                                      that DOF to activate)
#   left_hand_link → L_Elbow: 143mm  (previous mapping — geometrically unreachable,
#                                      was the root cause of stiff-elbow / 60mm error)
k1_joint_pick  = ['Trunk', 'Left_Shank', 'left_foot_link', 'Right_Shank', 'right_foot_link',
                  'Left_Arm_1', 'Left_Arm_3', 'Right_Arm_1', 'Right_Arm_3']
...
if __name__ == "__main__":
    ...
    for data_key in pbar:
        ...
        for iteration in range(500):
            ...
            arm_loss = diff[:, :, 5:].norm(dim=-1).mean()
            loss = leg_loss + arm_loss
            pbar.set_description_str(
                f"{iteration} legs={leg_loss.item()*1000:.1f} arms={arm_loss.item()*1000:.1f}"
            )
        ...
        # Phase 2: arm-only refinement with a fresh Adam optimizer so phase-1
        # Adadelta history (which compresses arm steps due to large sum gradients)
        # doesn't carry over.
        arm_optimizer = torch.optim.Adam([dof_pos_new], lr=0.02)
        for iteration in range(500):
            pose_aa_k1_new = torch.cat(
                [gt_root_rot[None, :, None], k1_rot_axis * dof_pos_new], dim=2
            )
            ...
            arm_loss.backward()
            dof_pos_new.grad[:, :, :2, :].zero_()    # freeze head
            dof_pos_new.grad[:, :, 10:, :].zero_()   # freeze legs
            arm_optimizer.step()
            dof_pos_new.data.clamp_(k1_fk.joints_range[:, 0, None], k1_fk.joints_range[:, 1, None])
            if iteration % 100 == 0:
                pbar.set_description_str(f"arm-refine {iteration} {arm_loss.item()*1000:.1f}mm")
